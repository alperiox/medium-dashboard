...
layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Div([], id='search_results')], className="row w-auto border") 
...
@dash.callback(
        Output("article-reading-time-claps", 'children'),
    ...
    if author != "None":
        ...
        for pub in publications:
            ...
            for idx, rtime in enumerate(pub_entries['Reading Time (mins)']):
                ...
                fig.add_trace(go.Scatter(x=tempdf['Date'], y=tempdf['Claps'], name=pub, legendgroup=pub, showlegend=s,
                                         hovertemplate="Claps: <b>%{y}</b><br>"
                                                      +f"Time: <b>{rtime} minutes</b><br>"
                                                      +"Date: %{x}",
                                         mode='markers', marker=dict(color=colormap[pub], size=rtime*2)))
...
@dash.callback(
        Output('most-published-pub-counts', 'children'),
        Input("author-dropdown", "value"),
        Input("authors_signal", 'data')
)
