...
layout = html.Div([
    ...
    html.Div([
        html.Div([], id='pub-claps', className='col'),
        html.Div([], id='pub-reading-time', className='col') 
    ], className='row')
...
@dash.callback(
        Output('pub-claps', 'children'),
        Input('publications-dropdown', 'value')
)
...
@dash.callback(
        Output('pub-reading-time', 'children'),
        Input('publications-dropdown', 'value')
)
...
@dash.callback(
        Output('publication-total-articles', 'children'),
        Input('publications-dropdown', 'value')
)
...
@dash.callback(
    Output('publications-dropdown', 'options'),
    Input('dataset_signal', 'data')
)
...
def avg_claps(signal):
    ...
    fig = go.Figure()
    barplot = go.Bar(x=avg_claps.publication_url, y=avg_claps.claps, marker={'color': px.colors.qualitative.Plotly},
                 texttemplate="%{y}",
                 hovertemplate="publication: <b>%{x}</b>")
    fig.add_trace(barplot)
    fig.update_layout(xaxis={'showticklabels': False},
                      title_text='Average claps')
...
@dash.callback(
        Output('avg-reading-time', 'children'),
        Input('dataset_signal', 'data')
)
def avg_reading_time(signal):
    ...
    fig = go.Figure()
    barplot = go.Bar(x=avg_reading_time.publication_url, y=avg_reading_time.reading_time, marker={'color': px.colors.qualitative.Plotly},
                 texttemplate="%{y}",
                 hovertemplate="publication: <b>%{x}</b>")
    fig.add_trace(barplot)
    fig.update_layout(xaxis={'showticklabels': False},
                      title_text='Average reading time')

    
    return dcc.Graph(figure=fig, id='avg-reading-time-graph')

@dash.callback(
    Output('num-articles-avg-earnings', 'children'),
    Input('dataset_signal', 'data')
)
def num_articles_avg_earnings(signal):
    ...
    barplot = go.Bar(x=num_articles.publication, y=num_articles.num_articles, showlegend=False,
               hovertemplate="Publication: <b>%{x}</b><br># articles: <b>%{y}</b>",
               marker={'color':px.colors.qualitative.Plotly},
               texttemplate="%{y}")

    pieplot = go.Pie(labels=earnings_group.publication_url, values=earnings_group.claps.round(),
                     marker={"colors": px.colors.qualitative.Plotly}, sort=False)
